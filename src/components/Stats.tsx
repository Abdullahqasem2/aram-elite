...
const Stats: React.FC = () => {
  ...
  return (
    <section className="glass-section text-white px-6 py-12 md:py-20">
      <div className="max-w-6xl mx-auto">
        <div className="mb-10 md:mb-16">
          <h2 className="font-cond text-3xl md:text-5xl font-bold text-white mb-2">Our Track Record</h2>
          <p className="font-arabic text-lg md:text-xl font-bold text-gold mb-3">أرقام تتحدث عن نجاحنا</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-8">
          {stats.map((stat, i) => (
            <div key={`stat-${i}`} className="text-center">
              <div className="font-cond text-4xl md:text-5xl lg:text-6xl font-bold text-gold mb-2">{stat.num}</div>
              <div className="font-cond text-xs md:text-sm lg:text-base font-bold text-white mb-1">{stat.label}</div>
              <div className="font-arabic text-xs text-gray-300">{stat.labelEn}</div>
            </div>
          ))}
        </div>
      </div>
